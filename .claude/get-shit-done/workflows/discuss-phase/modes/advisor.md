...
> `I:/Scripts/ML_Predictor2026_V2/.claude/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "I:/Scripts/ML_Predictor2026_V2/.claude/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Task()` in parallel:
   ...
   ```
   Task(
     prompt="First, read @I:/Scripts/ML_Predictor2026_V2/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
