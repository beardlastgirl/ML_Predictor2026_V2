...
- **Global scope**: `C:/Scripts/ML_Predictor2026_V2/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Scripts/ML_Predictor2026_V2/.gemini/notes/`)
...
2. Glob `C:/Scripts/ML_Predictor2026_V2/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/Scripts/ML_Predictor2026_V2/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Scripts/ML_Predictor2026_V2/.gemini/notes/` — works in any directory
